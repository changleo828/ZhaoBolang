...
const uchar RX_ADDRESS[RX_ADR_WIDTH]={0xFF,0xFF,0xFF,0xFF,0x05}; //发送地址

#define NRF_CSN   PBout(12)  //片选信号，推挽输出  
#define NRF_IRQ   PBin(5)  //IRQ主机数据输入,上拉输入
#define NRF_SCK   PBout(13)  //发送接收模式选择推挽输出
#define NRF_CE   	PBout(0)  //PC5 推完输出
...
{	
	NRF_CE=0;
		NRF24L01_Write_Reg(nRF_WRITE_REG+STATUS,1<<1);
...
/*********************************************/
/* 函数功能：24L01 powerdown                  */
/* 入口参数：wu              */
/* 返回值： wu                 */
/*********************************************/
void NRF24L01_PowerDown(void)
{    	 
	NRF24L01_Write_Reg(nRF_WRITE_REG+STATUS,0);
}
void NRF24L01_Sleep(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
	NRF_CE=0;//13.4mA
	NRF_CSN=1;//50uA
	NRF_SCK=1;// wu ying xiang
	NRF_MOSI=1;// wu ying xiang

  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5; 
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;//70uA
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(GPIOB, &GPIO_InitStructure);
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14; 
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;// wu ying xiang
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(GPIOB, &GPIO_InitStructure);
